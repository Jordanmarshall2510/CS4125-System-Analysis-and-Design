...
class Session():
    ...
    def init_database(host=None, user=None, password=None, database=None, remote=True):
        ...
        num_solar = 500
        num_wind = 100
        session_current_time = datetime.strptime(
            "2022-10-18 05:24:30", "%Y-%m-%d %H:%M:%S")
        ...
        if len(Session.db.select_info("session_id", Session.session_id)) == 0:
            Session.db.insert_session(Session.session_id, num_businesses, num_houses,
                                      num_infrastructure, num_vehicles, num_solar, num_wind, session_current_time)
    ...
    def init_simulation():
        ...
        builder.construct_businesses(
            (Session.db.select_info("num_businesses", Session.session_id))[0][0])
        builder.construct_houses(
            (Session.db.select_info("num_houses", Session.session_id))[0][0])
        builder.construct_infrastructure(
            (Session.db.select_info("num_infrastructure", Session.session_id))[0][0])
        builder.construct_vehicles(
            (Session.db.select_info("num_vehicles", Session.session_id))[0][0])
        builder.construct_solar_panels(
            (Session.db.select_info("num_solar", Session.session_id))[0][0])
        builder.construct_wind_turbines(
            (Session.db.select_info("num_wind", Session.session_id))[0][0])
    ...
    def run():
        # Initialise timer
        timestamp = datetime.strptime(str(Session.db.select_info(
            "session_current_time", Session.session_id)[0][0]), "%Y-%m-%d %H:%M:%S")
        for i in range(730):
            # Update the weather
            ...
            Session.db.insert_generation(
                timestamp, generation, Session.session_id)
