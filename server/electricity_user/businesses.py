...
class Business(ElectricityUser):
    ...
    path = os.path.dirname(os.path.realpath(__file__)).split(
        "electricity_user")[0] + "config.json"

    with open(path) as json_file:
        conf = json.load(json_file)

    # Average house value in Ireland. Units in euro.
    average_property_value_per_occupant = conf["electricity_user"][
        "businesses"]["average_property_value_per_occupant"]
    ...
    def update(self, date: datetime) -> int:
        ...
        # Weather
        total_usage += random.uniform(1, total_usage*self.weather_dictionary[Seasons.get_season(
        )] + total_usage*self.weather_dictionary[Weather.get_weather()])
        # Time
    ...
    def generate_users(number_of_businesses: int) -> list:
        ...
        for x in range(number_of_businesses):
            ...
            business = Business("B" + str(business_counter), Business.get_random_electricity_usage(number_of_occupants),
                                Business.get_random_property_value(number_of_occupants), Business.get_random_property_size(number_of_occupants), number_of_occupants)
    ...
    def get_random_property_value(number_of_occupants):
        ...
        valuePerOccupant = random.randint(Business.average_property_value_per_occupant - property_value_tolerance,
                                          Business.average_property_value_per_occupant + property_value_tolerance)
    ...
    def get_random_property_size(number_of_occupants):
        ...
        valuePerOccupant = random.randint(Business.average_sqm_per_occupant - property_sizeTolerance,
                                          Business.average_sqm_per_occupant + property_sizeTolerance)
        value = valuePerOccupant * number_of_occupants
        return value
    ...
    def get_random_electricity_usage(number_of_occupants):
        ...
        daily_average_usage = random.randint(Business.average_electricity_usage - electricity_usage_tolerance,
                                             Business.average_electricity_usage + electricity_usage_tolerance)/4
