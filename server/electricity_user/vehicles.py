...
class Vehicle(ElectricityUser):
    ...
    def __init__(self, vehicle_id, vehicle_value, battery_capacity, range, number_of_new_passengers):
        ...
        self.real_range = int(self.max_range - (self.max_range *
                              self.average_new_passenger_range_cost * self.number_of_new_passengers))
    ...
    def generate_users(number_of_vehicles: int) -> list:
        ...
        for i in range(number_of_vehicles):
            # Generate Values
            vehicle_id = "C" + str(i)
            vehicle_value = random.randint(
                Vehicle.average_vehicle_value - vehicle_value_tolerance, Vehicle.average_vehicle_value + vehicle_value_tolerance)
            battery_cap = random.randint(Vehicle.average_battery_capacity - battery_capacity_tolerance,
                                         Vehicle.average_battery_capacity + battery_capacity_tolerance)
            vehicleRange = random.randint(
                Vehicle.average_range - vehicle_range_tolerance, Vehicle.average_range + vehicle_range_tolerance)
            passengers = random.randint(0, 5)

            # Generate Vehicle
            vehicle = Vehicle(vehicle_id, vehicle_value,
                              battery_cap, vehicleRange, passengers)
