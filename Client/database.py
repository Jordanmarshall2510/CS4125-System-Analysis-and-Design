...
class Database:
	...
	def selectGeneratedPowerHistory(self, type):
		self.cur.execute("SELECT power_used FROM generators WHERE type = ? ORDER BY time", [type])
		return self.cur.fetchall()
	...
	def selectTotalGeneratedPowerHistory(self):
		self.cur.execute("SELECT SUM(power_used) AS total FROM generators GROUP BY time")
		return self.cur.fetchall()
